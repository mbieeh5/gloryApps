...
public class AdapterDataRedeem extends RecyclerView.Adapter<AdapterDataRedeem.DataViewHolder> {
    ...
    @NonNull
    @Override
    public AdapterDataRedeem.DataViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_layout_redeem_point, parent, false);
        return new DataViewHolder(v);
    }
    @Override
    public void onBindViewHolder(@NonNull AdapterDataRedeem.DataViewHolder holder, int position) {
        OutputRedeemClass oc = list.get(position);
                            ...
                            for(DataSnapshot ds : snapshot.getChildren()){
                                if(ds.child("point").exists() && ds.child("point").getValue() != null){
                                    ...
                                    String Nama = ds.child("nama").getValue().toString();
                                        if(PointSekarang <= jmlRedeem || jmlRedeem == 0){
